...
% Operators $\mu$ and $\mu'$ both take a sequence of \textit{cases} as their last
% argument, with each case of the form $|\ c\ \lowerc{\Gamma_A} \mapsto t$. Like
% with inductive data-type definitions, this is a somewhat inconvenient notation,
% so in the AST we will represent uses of $\mu$ and $\mu'$ more compactly. Given
% an inductive datatype definition $\indast{\Gamma_P}{\Gamma_I}{\Sigma}$ for
% type $I$, the expression $\mumat{t\ @P}{(|\ c_i\ \lowerc{\Gamma_{A_i}} \mapsto
%   t_i)_{i=1..\lenc{\Sigma}}}$ will be written as
% $\mu'(t,P,(\absu{\lambda\Lambda}{\Gamma_{A_i}}{f_i})_{i=1..\lowerc{\Sigma}})$
% % TODO erasure noation!

% The
% \textit{pattern} of the case is $c\ \vars{x}$. Given some (well-formed)
% inductive datatype definition $\indast{\Gamma_P}{\Gamma_I}{\Sigma}$ of some
% type $I$, we say that some pattern is well-formed ($WF\mhyph Pat(I, c\ \vars{x}, \Gamma_A)$) when

% \begin{itemize}
% \item $c$ is a data constructor of $I$: $c \in \Sigma$
% \item $\Gamma_A$ is a context binding $\vars{x}$ to the appropriate types:
%   $\lowerc{\Gamma_A} = \vars{x}$
%   % $\Gamma_I,\Gamma_P,\Sigma,\Gamma_A \decsyn c\
%   % \lowerc{\Gamma_P}\ \vars{x} : I\ \lowerc{\Gamma_P}\ \vars{s}$, for some
%   % $\vars{s}$
% \item $\vars{x}$ matches the shape of the type $T$ of $c\ \lowerc{\Sigma}$ -- that
%   is, not only does it contain an argument for each quantification in $T$, but
%   each argument is given erased / unerased according to the corresponding
%   quantifiers in $T$.
% \end{itemize}

% We furthermore say that a set of patters is well-formed ($WF\mhyph Pats(I, (c\
% \vars{x}_i, \Gamma_{A_i})_{i=1..\lenc{\Sigma}})$) when

% \begin{itemize}
% \item Each pattern is well-formed: $WF\mhyph Pat(I,c_i\ \vars{x}_i,
%   \Gamma_{A_i})$
% \item The constructors in the patterns are distinct: for all $i,j$, $c_i = c_j$
%   means $i = j$
% \end{itemize}
...
Let $\Gamma_P,\Gamma_I,$ and $\Sigma$ be contexts such that $\Gamma_I$
...
\begin{itemize}
  ...
  \item No other previously defined inductive datatypes $I'$ nor constructors
    $c'_1..c'_{n'}$ occur anywhere in $\Gamma_P$,$\Gamma_I$, or $\Sigma$
\end{itemize}
...
\begin{figure}[h]
  ...
  \[
    \begin{array}{ccc}
      \infer
      { \llbracket \ann{T}{★}\ |\ T \to ★ \rrbracket }
      { }
      & \infer
        { \llbracket \ann{T}{\abs{\Pi}{s}{C}{K_I}\ |\ \abs{\Pi}{s}{C}{K}} \rrbracket}
        { \llbracket \ann{T\ s}{K_I}\ |\ K \rrbracket }
    \end{array}
  \]
\end{figure}
...
some inductive data-type $I$ is permissible. We
write this judgment as $\llbracket \ann{T}{K_I} | K \rrbracket$, which should be read
``the type $T$ of a constructor of $I$ can be eliminated with a motive of kind
$K$'' (where $I$ is usually obvious by the context). This judgment is defined by
the simple rules in Figure \ref{fig:valid-elim-kind}. For example, a valid
elimination kind for the type $Vec\ \cdot X$ is
$\abs{\Pi}{n}{Nat}{\abs{\Pi}{x}{Vec\ \cdot X\ n}{X}}$

\subsection{Valid Branch Type}

Another piece of kit we need is to ensure that, in a pattern-matching
...
\rrbrace^P$ to indcate the type coressponding to the (possibly partially
applied) constructor $c$ and its type $T$ of an inductive data-type $I$ (that
is, we assume $T$ is the type of a constructor of $I$). Computing this type is
defined recursively by the following set of equations:

\[
  \begin{array}{rcl}
    \llbrace c : I\ \vars{T}\ \vars{s} \rrbrace^P
    & = & P\ \vars{s}\ c
    \\ \llbrace c : \abs{\forall}{x}{C}{T} \rrbrace^P
    & = & \abs{\forall}{x}{C}{\llbrace c\ x : T \rrbrace }
    \\ \llbrace c : \abs{\Pi}{x}{C}{T} \rrbrace^P
    & = & \abs{\Pi}{x}{C}{\llbrace c\ x : T \rrbrace }
  \end{array}
\]

\noindent where we leave implicit the book-keeping required to separate the
instantiated parameters $\vars{T}$ from the indicies $\vars{s}$. When the type
of $c$ is obvious from context we will write $\llbrace c \rrbrace^P$
...
quantifiers corresponding to those of the arguments to the data constructor for
the pattern for the branch.
...
\begin{figure}[h]
  ...
  \[
    \infer
    { \wfpat{\Gamma,\Gamma_{\mu}}{\indast{p}{\Gamma_I}{\Sigma}}{\vars{T}}{\mu'(t,P,t_{i=1..n})}
    }
    { \Gamma \vdash P : K
      ...
      \quad (\Gamma,\Gamma_{\mu} \decchk t_i : \llbrace c_i\ \vars{T} \rrbrace^P)_{i=1..n}
    }
  \]
\end{figure}
...
$\mumat(t,P,t_{i=1..n})$ is well-formed. We check that the motive $P$ is
well-kinded at kind $K$, that an inductive data-type $I$ instantiated with
parameters $T$ can be eliminated to a type of kind $K$, that the branches $t_i$
account for each of the constructors in $\Sigma$, and that the given parameters
$\vars{T}$ matches the number we are expecting $p$ from the given inductive
...
Assuming that an inductive definition $\indast{p}{\Gamma_I}{\Sigma}$ is
