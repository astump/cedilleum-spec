...
tilanguages like Agda, Idris, or Coq. Some key differences to note are:
...
\texttt{rec/type} lexically scoped to the body of the μ-expression and is
\textit{automatically generated} by Cedilleum by using the name of the recursive
function \texttt{rec} (which is given by the user) bound by μ. Why introduce this new type? For recursive
...
``recursive-occurence'' types appear in the types of constructor sub-data (such
as \texttt{p} in the example) in patterns introduced by μ, replacing all
...
checking that usually involves making sure that arguments to recursive calls are
...
\begin{figure}[h]
\begin{verbatim}
pred' : ∀ R: ★. Rec ·Nat ·R ➾ R ➔ R
  = Λ R. Λ recR. λ r. μ'<recR> r {| zero ➔ r | succ r' ➔ r'}.

pred : Nat ➔ Nat = pred' -Nat/mu .

minus' ∀ R: ★. Rec ·Nat ·R ➾ R ➔ Nat ➔ R
  = Λ R. Λ recR. λ m. λ n. μ rec. n @(λ _: Nat. R) {
  | zero   ➔ m
  | succ n' ➔ pred' -recR (rec n')
  }.
...
divide : Nat ➔ Nat ➔ Nat
  ...
  | zero   ➔ zero
  | succ n' ➔
      ite (lt (succ n') d) zero
        (succ (rec (minus' -rec/mu n' (pred d))))
  }.
\end{verbatim}
...
below. The type \texttt{Rec ·Nat ·R} in its type signature is the type of
\textit{witnesses} that terms of type \texttt{R} can be pattern-matched upon as
if they had type \texttt{Nat}; in the definition of \texttt{pred'}, this witness
is given name \texttt{recR}. All such witnesses are introduced in only one of
...
\texttt{Rec ·Nat ·Nat}; and \textit{locally} for each recursive-occurence type
introduce by μ (in the body of \texttt{divide}, term \texttt{rec/mu} has type
\texttt{Rec ·Nat ·rec/type}). In the definition of \texttt{pred'}, the notation
\texttt{μ'<recR>} indicates that the witness \texttt{recR} is given explicitly
...
\texttt{R} is specialized to \texttt{Nat} itself with \texttt{Nat/mu}.

Next we define \texttt{minus'}. One intuition for understanding the type
signature of \texttt{minus'} (and \texttt{pred'} before it) is that it says
``this function will never increase the size of its \texttt{R} argument''. That
is to say, to return a term of type \texttt{R}, \texttt{minus'} can only return
its argument or some sub-data produced by destructing it through
pattern-matching. In \texttt{pred'} this is done only once; in \texttt{minus'}
it is done \texttt{n} times by recursion on subtrahend \texttt{n}, invoking
\texttt{pred'} each time.

Finally, we turn to the definition of \texttt{divide} itself. At a high level,
we recurse on dividend \texttt{n}, and in the step case test whether \texttt{n}
(now known to have the form \texttt{succ n'}) is less than the divisor
\texttt{d}: if it is we return \texttt{zero}, otherwise we recurse after
subtracting. First, consider that \texttt{succ} has type \texttt{Nat
  ➔ Nat}, but here it is applied to argument \texttt{n'} of type
\texttt{rec/type}. The reason that this is well-typed is that
recursive-occurence types are always \texttt{subtypes} of the inductive
data-type from which they came -- here, the type of \texttt{n'} is promoted to
\texttt{Nat} in the application \texttt{succ n'}. Second, as mentioned
before recursive function \texttt{rec} requires an argument of type
\texttt{rec/type}, so must use \texttt{minus'} (passing the generated, locally
bound witness \texttt{rec/mu}) so that after subtraction we have a value of the
appropriate type. Finally, note how this definition of division is terminating
even when divisor \texttt{d} is \texttt{zero} -- in that case the expression
\texttt{minus' -rec/mu n' (pred d)} reduces to \texttt{n'}, which is
\textit{still} smaller than the expression this recursive case started with,
\texttt{succ n'}. Put another way: the ``naive'' expression \texttt{rec (minus (succ
  n') d)} which is potentially non-terminating is not accepted by Cedilleum
\textit{because it is ill-typed}.
...
case, pattern \texttt{zero} is substituted in for \texttt{x} in the motive, and
