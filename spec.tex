...
borrows heavily from the conventions of the Coq
...
A kind $K$ is a $p$-arity if it can be written as $\absu{\Pi}{\Gamma_P}{K'}$,
where $|\Gamma_P| = p$ and $\Pi\ \Gamma_P$ represents the quantification of all
term and type variables in $\Gamma_C$. For an inductive definition
...
We say that a type $T$ of a constructor $I$ satisfies the positivity condition
when
