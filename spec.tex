...
\item $t\ \Gamma$ and $T\ Gamma$ represent the application of term $t$ (resp.
  type $T$) to each variable in $\Gamma$ in order of appearance. The relevance
  ...
  always be disambiguated by the type of $t$
...
Notation $\indsche{I}{\Gamma_P}{K}{R}{\Sigma}{\Gamma_G}$ represents a declaration of
an inductive datatype named $I$ where:
\begin{itemize}
\item $\Gamma_P$ is the context of parameters;
\item $K$ is the index-sort of $I$; that is to say type $I\ \Gamma_P$ has kind
  $K$;
\item $R$ is a (fresh) type variable of kind $K$, serving as a placeholder for
...
\[
  \indsche{Vec}{\ann{A}{★}}{Nat ➔ ★}{R}{\Sigma}{\Gamma_G}
\]
\noindent where
\[
  \begin{array}{lcl}
    ...
    & =
    & \begin{array}{lcl}
        \texttt{Is/Vec} : & \texttt{★ ➔ (Nat ➔ ★) ➔ ★}
        \\ \texttt{is/Vec} : & \texttt{∀ A:★. Is/Vec ·A ·(Vec ·A)}
        \\ \texttt{to/Vec} : & \texttt{∀ A:★. ∀ R: Nat ➔ ★.Is/Vec ·A ·R ➾ ∀ n:Nat.R n ➔ Vec ·A n = λ x.x}
      \end{array}
  \end{array}
\]

In the above definition of $\Gamma_P$, understand that
\begin{itemize}
  \item \texttt{Is/}$I$ is an automatically-generated type 
\end{itemize}

(where again $\Gamma_G$ will be explained below). Note that the
programmer writes in the concrete syntax \texttt{Vec n} for the type of the
recursive argument to constructor \texttt{vcons}; it is straightforward to
translate this more intuitive form of datatype definition to the internal one in
which recursive occurrences are marked explicitly with a fresh type variable
$R$.
\subsection{Well-formedness of Datatype Definition}
For an inductive datatype definition
$\indsche{I}{\Gamma_P}{K}{\Sigma}{\Gamma_G}$ to be well-formed, it must satisfy
the following conditions:

\begin{itemize}
\item $I$ must have (well-formed) kind $\abs{\Pi}{\Gamma_P}{K}$
  Ensuring this is trivial from the concrete syntax
\item The types of each constructor $c \in \Sigma$ must be \textit{types of
    constructors of I} (c.f. Section \ref{ssec:inductive-aux-defs})
\item The types of each constructor $c \in \Sigma$ must satisfy the (non-strict)
