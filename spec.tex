...
cost) such terms back to the original type. As a motivating example, consider
...
this is witnessed by the proof \texttt{NatCast-id}, which holds by reflexivity.
...
\texttt{fact2}: the variable \texttt{n'} has type \texttt{rec/type}, and its
...
automatically solved, and coercions implicitly inserted, algorithmically by
...
development: how to support program reuse over different data-types at zero
...
either to re-implement the existing functionality for \texttt{Vec}, or to write
conversion functions between \texttt{List} and \texttt{Vec} to re-use existing
functionality. For this second option, such conversion functions usually must
...
\texttt{l2v'} that one could write in virtually any dependently typed language.
...
This is necessary (though not sufficient) in order to be able to prove
...
behaves extensionally like the identity function. With this, and with the term
...
From this point, the reuse is straightfoward: reuse in the other direction, of
\texttt{Vec} as \texttt{List}, is defined and two lemmas are needed, one
relating the vector index to list length after conversion, and one relating the
length of the list resutling from \texttt{append} to its two input lists. The
payoff comes at the last line of the code listing below: function
\texttt{vappend'} is definitionally equal to \texttt{append}, meaning our
conversions between the two data-structures has no run-time cost!
