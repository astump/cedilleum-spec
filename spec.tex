...
\item $\lenc{\Gamma}$ denotes the length of $\Gamma$ (the number of variables it
  binds)
\item $\lowerc{\Gamma}$ indicates converting $\Gamma$ to a sequence of variable
  arguments. When given as arguments to some expression $t$ whose type $T$ is
  known, it is assumed that each variable in the sequence is given with the
  right ``flavor'' of application (type, explicit, erased).
\item $\Gamma_{\mu}$ is a specially designated context tracking both global
  inductive datatype definitions as well local ``abstracted'' versions of these
  for uses of $\mu$ (see further below).
\end{itemize}
...
an inductive datatype definition $\indast{\Gamma_P}{\Gamma_I}{\Gamma_c}$ for
type $I$, the expression $\mumat{t\ @P}{(|\ c_i\ \lowerc{\Gamma_{A_i}} \mapsto
  t_i)_{i=1..\lenc{\Gamma_c}}}$ will be written as
$\mu'(t,P,(\absu{\lambda\Lambda}{\Gamma_{A_i}}{f_i})_{i=1..\lowerc{\Gamma_C}})$
% TODO erasure noation!

The
\textit{pattern} of the case is $c\ \vars{x}$. Given some (well-formed)
inductive datatype definition $\indast{\Gamma_P}{\Gamma_I}{\Gamma_C}$ of some
type $I$, we say that some pattern is well-formed ($WF\mhyph Pat(I, c\ \vars{x}, \Gamma_A)$) when

\begin{itemize}
\item $c$ is a data constructor of $I$: $c \in \Gamma_c$
\item $\Gamma_A$ is a context binding $\vars{x}$ to the appropriate types:
  $\lowerc{\Gamma_A} = \vars{x}$
  % $\Gamma_I,\Gamma_P,\Gamma_C,\Gamma_A \decsyn c\
  % \lowerc{\Gamma_P}\ \vars{x} : I\ \lowerc{\Gamma_P}\ \vars{s}$, for some
  % $\vars{s}$
\item $\vars{x}$ matches the shape of the type $T$ of $c\ \lowerc{\Gamma_c}$ -- that
...
\vars{x}_i, \Gamma_{A_i})_{i=1..\lenc{\Gamma_C}})$) when
